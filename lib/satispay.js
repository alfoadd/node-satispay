...
const satispay = {
	...
	obtain_key_id: function (public_key, token, callback) {
		//add promise support
		if (!callback) {
			return new Promise((resolve, reject) =>
				this.obtain_key_id(public_key, token, function (err, res) {
					err ? reject(err) : resolve(res)
				})
			)
		}
	...
	retrieve_consumer: (phone_number, extra_headers, callback) => {
		return make_call('GET', host[conf.sandbox], `/g_business/v1/consumers/${phone_number}`, null, null, extra_headers, callback)
	},
